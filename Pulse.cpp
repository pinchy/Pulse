...
void Pulse::_setMode(PulseMode_t mode)
{
    if (this->_mode != mode)
    {
        ...
        if (this->_mode != PULSE_INACTIVE)
            this->set(true); // start the mode by turning the led on
    }
}
...
void Pulse::tick()
{
    ...
    case PulseMode_t::PULSE_HEARTBEAT:
        ...
        {
            this->_lastInterval = millis();
            if (++this->_count >= (sizeof(this->_heartbeat) / sizeof(this->_heartbeat[0])))
                this->_count = 0;
    ...
    case PulseMode_t::PULSE_FLASH:
        ...
        {
            this->_lastInterval = millis();
            if (++this->_count >= (sizeof(this->_flash) / sizeof(this->_flash[0])))
                this->_count = 0;
            this->set(!(this->_count % 2)); // even is on
        }
        break;
    ...
    case PulseMode_t::PULSE_STROBE:
        ...
        {
            this->_lastInterval = millis();
            if (++this->_count >= (sizeof(this->_strobe) / sizeof(this->_strobe[0])))
                this->_count = 0;
            this->set(!(this->_count % 2)); // even is on
        }
        break;
    ...
    case PulseMode_t::PULSE_BLINK:
        ...
        {
            this->_lastInterval = millis();
            if (++this->_count >= (sizeof(this->_blink) / sizeof(this->_blink[0])))
                this->_count = 0;
            this->set(!(this->_count % 2)); // even is on
        }
        break;
